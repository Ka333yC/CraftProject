...
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
            if(!_world.IsEntityAliveInternal(entity)) { throw new EntityDestroyedException("Cant touch destroyed entity."); }
            if(_sparseItems[entity] > 0) { throw new Exception($"Component \"{typeof(T).Name}\" already attached to entity."); }
            if(!_world.IsAlive()) { throw new WorldDestroyedException(); }
#endif
			int idx;
