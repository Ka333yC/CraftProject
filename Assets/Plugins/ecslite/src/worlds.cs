...
namespace Leopotam.EcsLite {
    ...
    public class EcsWorld
	{
        ...
        public void Destroy () {
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
            if (CheckForLeakedEntities ()) { throw new EmptyEntityException($"Empty entity detected before EcsWorld.Destroy()."); }
#endif
            _destroyed = true;
...
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
            if (entity < 0 || entity >= _entitiesCount) { throw new EntityDestroyedException("Cant touch destroyed entity."); }
#endif
        ...
        (EcsFilter, bool) GetFilterInternal(Mask mask, int capacity, bool isHashed)
        {
            var hash = mask.Hash;
            EcsFilter filter;
            if(isHashed)
            {
                var exists = _hashedFilters.TryGetValue(hash, out filter);
                if(exists)
                {
                    return (filter, false);
                }
    ...
    [Il2CppSetOption (Option.ArrayBoundsChecks, false)]
        ...
        public sealed class Mask {
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public EcsFilter End(int capacity = 512, bool isHashed = true) {
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
                if (_built) { throw new Exception ("Cant change built mask."); }
                _built = true;
#endif
                ...
                var (filter, isNew) = _world.GetFilterInternal (this, capacity, isHashed);
                if (!isNew) { Recycle (); }
                return filter;
            }
