        ...
        public virtual void Init () {
            ...
            foreach (var system in _allSystems) {
                if (system is IEcsPreInitSystem initSystem) {
                    ...
                    if (worldName != null) { throw new EmptyEntityException($"Empty entity detected in world \"{worldName}\" after {initSystem.GetType ().Name}.PreInit()."); }
#endif
                }
            }
            foreach (var system in _allSystems) {
                if (system is IEcsInitSystem initSystem) {
#if LEOECSLITE_PROFILER
                    Profiler.BeginSample($"{initSystem.GetType().Name}.Init()");
#endif
...
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
                    var worldName = CheckForLeakedEntities (this);
                    if (worldName != null) { throw new EmptyEntityException($"Empty entity detected in world \"{worldName}\" after {initSystem.GetType ().Name}.Init()."); }
#endif
                }
            }
#if DEBUG
...
#if LEOECSLITE_PROFILER
                Profiler.BeginSample($"{_runSystems[i].GetType().Name}.Run()");
#endif
...
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
                var worldName = CheckForLeakedEntities (this);
                if (worldName != null) { throw new EmptyEntityException($"Empty entity detected in world \"{worldName}\" after {_runSystems[i].GetType ().Name}.Run()."); }
#endif
            }
            for (int i = 0, iMax = _postRunSystems.Count; i < iMax; i++) {
#if LEOECSLITE_PROFILER
                Profiler.BeginSample($"{_postRunSystems[i].GetType().Name}.PostRun()");
#endif
...
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
                var worldName = CheckForLeakedEntities (this);
                if (worldName != null) { throw new EmptyEntityException($"Empty entity detected in world \"{worldName}\" after {_postRunSystems[i].GetType ().Name}.PostRun()."); }
#endif
            }
        }
...
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
                    var worldName = CheckForLeakedEntities (this);
                    if (worldName != null) { throw new EmptyEntityException($"Empty entity detected in world \"{worldName}\" after {destroySystem.GetType ().Name}.Destroy()."); }
#endif
                }
            }
            for (var i = _allSystems.Count - 1; i >= 0; i--) {
...
#if DEBUG && !LEOECSLITE_NO_SANITIZE_CHECKS
                    var worldName = CheckForLeakedEntities (this);
                    if (worldName != null) { throw new EmptyEntityException($"Empty entity detected in world \"{worldName}\" after {postDestroySystem.GetType ().Name}.PostDestroy()."); }
#endif
                }
            }
            _worlds.Clear ();
